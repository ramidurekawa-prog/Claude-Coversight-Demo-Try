...
| Auth | Planned signed persona cookie | better-auth (email+password, argon2id, invite-only, sessions in Postgres), owner/gm memberships, GM location scopes, `skc_admins` | **Adopt better-auth as-is.** Demo users are seeded with known passwords and the sign-in page offers persona buttons; tenancy stays in the repository layer. |
